...
from experimental3.common_math import BF16Ptr, rms_reduce_bf16, inv_rms_from_sum_sq
...
from sampling import (
    ...
    SamplingConfig,
    SamplingRng,
    sample_sharded_topk_minp_temperature,
)
...
struct MiniMaxM27ButterQuant[tp: Int, Pool: BurstThreadPool = BurstPool[]](Movable):
    ...
    def __init__(out self,
        ...
        self.sampling_config = SamplingConfig.greedy()
        self.sampling_rng = SamplingRng()
    ...
    def set_sampling(
        ...
        self.sampling_config = SamplingConfig(
            temperature=temperature, top_k=top_k, min_p=min_p)
        if seed != UInt64(0):
            self.sampling_rng = SamplingRng(seed)
    ...
    def forward(
        ...
        var t_sample0 = Int(perf_counter_ns())
        ...
        var best_idx = sample_sharded_topk_minp_temperature[
            ...
            logit_ptrs,
            C.GENERATION_VOCAB_SIZE,
            self.sampling_config,
            self.sampling_rng,
        )
        if self.sampling_config.is_greedy():
            sample.add(self.profile.phase("lm_argmax"), PhaseTiming.opaque(Int(perf_counter_ns()) - t_sample0))
        else:
            sample.add(self.profile.phase("lm_sample"), PhaseTiming.opaque(Int(perf_counter_ns()) - t_sample0))
        logit_lease^.release()
