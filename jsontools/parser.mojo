...
@always_inline
def simd_string_stop[w: Int](block: SIMD[DType.uint8, w]) -> SIMD[DType.bool, w]:
    return block.eq(QUOTE) | block.eq(BACKSLASH) | block.le(Byte(31))
...
struct Parser[origin: Origin, simd_width: Int = 16, max_depth: Int = 256]:
    ...
    def __init__(out self, data: Span[Byte, Self.origin]):
        comptime assert Self.simd_width > 0 and Self.simd_width <= 64, (
            "Parser simd_width must be in [1, 64]"
        )
        comptime assert Self.max_depth > 0, "Parser max_depth must be positive"
        self.data = data
        self.pos = 0
        self.depth = 0
    ...
    def append_escape(mut self, mut out: List[Byte]) raises ParseError:
        ...
        if cp >= 0xD800 and cp <= 0xDBFF:
            ...
            raise ParseError("unexpected low surrogate", self.pos)
        append_utf8(out, cp)
    ...
    def parse_string(mut self) raises ParseError -> String:
        ...
        while self.has_more():
            if self.remaining() >= Self.simd_width:
                var block = (ptr + self.pos).load[width=Self.simd_width]()
                var hits = simd_string_stop[Self.simd_width](block)
                if not any(hits):
            ...
            if b == BACKSLASH:
                ...
                raise ParseError("unescaped control character in string", self.pos - 1)
            else:
                out_bytes.append(b)
    ...
    def skip_array(mut self) raises ParseError:
        ...
        if self.depth > Self.max_depth:
            raise ParseError("max nesting depth exceeded", self.pos)
