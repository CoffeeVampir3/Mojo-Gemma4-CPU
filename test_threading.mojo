...
@fieldwise_init
struct DelayKernel(BurstKernel):
    var out_addr: UnsafePointer[Int, MutExternalOrigin]
    var job_id: Int
    var pauses: Int

    def execute(mut self):
        var sys = linux.linux_sys()
        for _ in range(self.pauses):
            sys.arch_cpu_relax()
        self.out_addr[] = self.job_id + 1
    ...
    for wi in range(7):
        ...
        for i in range(num_nodes):
            for j in range(pools[i].get_capacity()):
                node_kernels[i][j].pauses = pauses
