...
from .tokenizer import (
    ...
    simd_spaces,
    split_numbers,
    consume_ascii_then_codepoint_run,
    try_contraction_at,
    ASCII_SPACE,
    ASCII_APOSTROPHE,
)
...
def consume_letter_run(data: Span[Byte, _], start: Int, n: Int, ctx: UnicodeContext) -> Int:
    return consume_ascii_then_codepoint_run[
        is_ascii_letter, simd_ascii_letters, is_unicode_letter_cp,
    ](data, start, n, ctx)
...
def consume_number_run(data: Span[Byte, _], start: Int, n: Int, ctx: UnicodeContext) -> Int:
    return consume_ascii_then_codepoint_run[
        is_ascii_digit, simd_ascii_digits, is_unicode_number_cp,
    ](data, start, n, ctx)
...
def try_contraction(data: Span[Byte, _], pos: Int, n: Int) -> Int:
    return try_contraction_at[case_insensitive=False, accept_unicode_quote=False](data, pos, n)
...
def pre_tokenize_bytelevel_span(
    ...
    while i < end:
        ...
        # ?\p{L}+
        if b == ASCII_SPACE and i + 1 < end and is_letter_start_at(data, i + 1, end, ctx):
        ...
        # ?\p{N}+
        if b == ASCII_SPACE and i + 1 < end and is_number_start_at(data, i + 1, end, ctx):
