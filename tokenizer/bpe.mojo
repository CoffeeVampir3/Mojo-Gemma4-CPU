...
comptime BPE_PAIR_TOKEN_ID_MAX = 0xFFFFFFFF


@always_inline
def token_id_fits_pair_key(id: Int) -> Bool:
    return id >= 0 and id <= BPE_PAIR_TOKEN_ID_MAX


@always_inline
def pack_pair_ids(left: Int, right: Int) -> UInt64:
    debug_assert(token_id_fits_pair_key(left), "left BPE token id does not fit pair key")
    debug_assert(token_id_fits_pair_key(right), "right BPE token id does not fit pair key")
