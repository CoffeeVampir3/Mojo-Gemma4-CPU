...
from kernels.reductions import (
    AddIntoDest, allreduce, reduce_writeback, broadcast, allgather,
)
...
def test_allreduce_exact[tp: Int](n: Int, label: String, inline_max_bytes: Int):
    ...
    for r in range(tp):
        ...
        dst.ptrs[r] = out[r]
    allreduce[BF16, tp](src, dst, pools, inline_max_bytes)
    for i in range(n):
...
def test_broadcast_exact[tp: Int](n: Int, label: String, inline_max_bytes: Int):
    ...
    for r in range(tp):
        src.ptrs[r] = raw[r].as_immutable()
        dst.ptrs[r] = out[r]
    broadcast[BF16, tp](src, dst, pools, inline_max_bytes)
    for i in range(n):
        for r in range(1, tp):
...
def accuracy_crosspath[tp: Int](n: Int, label: String):
    ...
    allreduce[BF16, tp](src, dst_i, pools_a, 16384)
    allreduce[BF16, tp](src, dst_p, pools_b, 0)
