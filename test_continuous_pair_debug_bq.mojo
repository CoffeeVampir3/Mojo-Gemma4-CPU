...
def load_and_run[
    ...
    while sched.pending_work():
        ...
        steps += 1
        var slot_line = String(t"step {steps}:")
        for s in range(len(sched.schedule.slots)):
            var sid = sched.schedule.slots[s].seq_id
            var rid = sched.schedule.slots[s].request_id
            var bp = sched.schedule.slots[s].base_pos
            var nt = sched.schedule.slots[s].n_tokens
            var em = 1 if sched.schedule.slots[s].emit else 0
            slot_line += String(t" [req{rid} seq{sid} base{bp} n{nt} e{em}]")
        var n_copies = len(sched.schedule.copies)
        if n_copies > 0:
            slot_line += String(t" copies={n_copies}")
        print(slot_line)
        for k in range(num_requests):
            var n_gen = len(sched.requests[request_ids[k]].generated)
            if n_gen > 0 and n_gen <= 4:
                var tok_line = String(t"  req{k} generated:")
                for g in range(n_gen):
                    var tid = sched.requests[request_ids[k]].generated[g]
                    tok_line += String(t" {tid}")
                print(tok_line)
...
def main():
    print("Continuous-batching pair DEBUG test (bq): two long prompts")
